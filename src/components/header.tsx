    ...
    return (
        <header className="mb-8 relative">
            <nav className="flex flex-wrap justify-between items-center px-4 py-4 lg:px-8">
                <Link href="/" className="comic-text text-3xl random-rotate wobble z-50 animate-slideInFromLeft">GDG DA-IICT</Link>
                <button className="lg:hidden z-50 text-black" onClick={toggleMenu}>{isMenuOpen ? <X size={24} /> : <Menu size={24} />}</button>
                <ul className={`fixed inset-0 flex flex-col items-center justify-center space-y-6 lg:space-y-0 lg:space-x-4 lg:flex-row lg:relative lg:bg-transparent z-40 animate-slideInFromRight ${isMenuOpen ? 'flex bg-white' : 'hidden lg:flex'}`}>
